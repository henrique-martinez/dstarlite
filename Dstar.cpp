...
//#include <iostream.h>
bool isinf(double d){return d==INFINITY;}
double fmin(double d1,double d2){return min(d1,d2);}
...
int Dstar::computeShortestPath() {
  ...
  while ((!openList.empty()) && 
    ...
    if (k_old < calculateKey(u)) { // u is out of date
      ...
      setG(u,INFINITY);//cout<<INFINITY<<endl;
      getPred(u,s);
      for (i=s.begin();i != s.end(); i++) {
        updateVertex(*i);
      }
...
double Dstar::cost(state a, state b) {
  ...
  if (xd+yd>1) scale = sqrt(2.0);//M_SQRT2;
